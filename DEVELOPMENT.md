...
**Переход на GPU — теперь один флаг:**
- `generate.py` уже умеет оба режима. Переключатель — ENV `GEN_PROVIDER`:
  - `GEN_PROVIDER=api` (дефолт, старт) — генерация через fal (nano-banana + CodeFormer).
  - `GEN_PROVIDER=gpu` — один вызов RunPod Serverless (оба слоя на своём железе, ~11× дешевле).
- Для GPU-режима добавить ENV: `RUNPOD_ENDPOINT` (https://api.runpod.ai/v2/<id>/runsync), `RUNPOD_KEY`.
- Код GPU-образа — `worker/runpod_handler.py` (заготовка handler’а; контракт вход/выход уже совпадает с generate.py). Остаётся встроить реальные модели (SUPIR/CodeFormer) и собрать Docker-образ.
- Стадии 1 (ИИ-глаза) и 3 (проверка) остаются на дешёвом API — на GPU идёт только тяжёлая генерация.

**Итог:** при росте потока переключение API→GPU = поменять одну ENV-переменную, без правки кода.
